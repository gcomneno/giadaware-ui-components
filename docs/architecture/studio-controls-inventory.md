...
| 6 | Ordered asset or record editor | Atelier-Kit #225 characterizes Gallery and Meta ordered rows; Giada UI #30 introduces `EditableList`, `EditableListRow`, and `ReorderActions`; Giada UI #35 adds `ReorderAnnouncement` for confirmed reorder outcomes. | Shared structure, reorder presentation, and the visually hidden announcement companion are implemented. Row schemas, identity, cardinality, dirty tracking, focus, form encoding, defaults, validation, persistence, localization, resolved messages, event keys, and duplicate-live-region avoidance remain consumer-owned; Atelier-Kit adoption is separate work. |
...
2. The ordered editor's shared structure, reorder presentation, and confirmed-outcome announcement companion are implemented through #30 and #35. Gallery and Meta adoption remains separate consumer work, while their schemas, cardinality, dirty tracking, focus, form naming, defaults, validation, persistence, resolved messages, event keys, and duplicate-live-region avoidance remain outside Giada UI.
