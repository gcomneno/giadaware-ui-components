...
| 6 | Ordered asset or record editor | Atelier-Kit #225 characterizes Gallery and Meta ordered rows; Giada UI #30 introduces `EditableList`, `EditableListRow`, and `ReorderActions`; Giada UI #35 adds `ReorderAnnouncement` for confirmed reorder outcomes; Giada UI #36 adds consumer-owned non-live position context descriptions for `ReorderActions`; Giada UI #37 adds optional row-local pointer drag handles. | Shared structure, keyboard-equivalent reorder presentation, optional handle-only pointer drag intent, optional action-group position descriptions, and the visually hidden announcement companion are implemented. Row schemas, identity, cardinality, candidate calculation, position and total calculation, dirty tracking, focus, form encoding, defaults, validation, mutation, persistence, localization, resolved messages, event keys, and duplicate-live-region avoidance remain consumer-owned; Atelier-Kit adoption is separate work. |
