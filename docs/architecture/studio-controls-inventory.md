...
| 6 | Ordered asset or record editor | Atelier-Kit #225 characterizes Gallery and Meta ordered rows; Giada UI #30 introduces `EditableList`, `EditableListRow`, and `ReorderActions`; Giada UI #35 adds `ReorderAnnouncement` for confirmed reorder outcomes; Giada UI #36 adds consumer-owned non-live position context descriptions for `ReorderActions`. | Shared structure, reorder presentation, optional action-group position descriptions, and the visually hidden announcement companion are implemented. Row schemas, identity, cardinality, position and total calculation, dirty tracking, focus, form encoding, defaults, validation, persistence, localization, resolved messages, event keys, and duplicate-live-region avoidance remain consumer-owned; Atelier-Kit adoption is separate work. |
...
2. The ordered editor's shared structure, reorder presentation, non-live position-context description hook, and confirmed-outcome announcement companion are implemented through #30, #35 and #36. Gallery and Meta adoption remains separate consumer work, while their schemas, cardinality, position and total calculation, dirty tracking, focus, form naming, defaults, validation, persistence, resolved messages, event keys, and duplicate-live-region avoidance remain outside Giada UI.
...
It does not render a semantic `label`, associate controls, generate control
IDs, apply native `required`, translate strings, render validation messages or
provide form-level required/optional legends. Those responsibilities remain
with the consumer and its server-validation contract.

The component is supported by type-contract, deterministic SSR, DOM-preserving
hydration, browser-state and Axe accessibility tests.
