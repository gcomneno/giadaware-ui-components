...
import {
	IMAGE_ATTACHMENT_CONTROL_HYDRATION_DROPZONE,
	IMAGE_ATTACHMENT_CONTROL_HYDRATION_LABELS,
	IMAGE_ATTACHMENT_CONTROL_HYDRATION_SSR_BODY
} from '../fixtures/image-attachment-control-hydration-contract.js';
...
test('hydrates the controlled image attachment DOM without recovery and preserves focus transitions', async () => {
	...
	try {
		...
		const serverDropzone = container.querySelector('[data-dropzone="true"]');
		...
		if (!(serverProbe instanceof HTMLElement) || !(serverControl instanceof HTMLElement) || !(serverInput instanceof HTMLInputElement) || !(serverImage instanceof HTMLImageElement) || !(serverDropzone instanceof HTMLElement) || !(serverRemove instanceof HTMLButtonElement)) throw new TypeError('The shared hydration body is incomplete.');
		...
		expect(container.querySelector('[data-dropzone="true"]')).toBe(serverDropzone);
		...
		expect(serverDropzone).toHaveAttribute('role', 'group');
		expect(serverDropzone).toHaveAttribute('data-drop-active', 'false');
		expect(serverDropzone).toHaveAttribute('data-drop-rejected', 'false');
		expect(serverDropzone).toHaveTextContent(IMAGE_ATTACHMENT_CONTROL_HYDRATION_DROPZONE.instructions);
