...
try {
	...
	await writeFile(
		...
		[
			...
			'}).body;',
			'',
			"if (!imageLightboxMarkup.includes('<dialog') || !imageLightboxMarkup.includes('Packed image preview') || !imageLightboxMarkup.includes('Packed image')) {",
	...
	await writeFile(
		...
		[
			...
			"const imageLightboxProps: ImageLightboxProps = { open: false, onopenchange: (open) => { void open; }, src: '/packed.jpg', alt: 'Packed image', labels: imageLightboxLabels };",
	...
	await writeFile(
		...
		[
			...
			"import type { AsyncOperationPanelProps, AsyncOperationState, ButtonProps, ButtonVariant, FieldLabelProps, FormActionsAlign, FormActionsProps, ImageAttachmentControlLabels, ImageAttachmentIntent, ImageAttachmentState, PageIntroProps, PanelHeadingLevel, PanelProps, SurfaceProps } from 'giadaware-ui-components/studio';",
			"import type { ComponentProps, Snippet } from 'svelte';",
			"import { ImageLightbox } from 'giadaware-ui-components/visitor';",
			"import type { ImageLightboxLabels, ImageLightboxProps } from 'giadaware-ui-components/visitor';",
			'',
