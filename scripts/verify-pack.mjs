...
try {
	...
	await writeFile(
		...
		[
			...
			"\tconst packedCandidate = { sourceId: 'packed-hero', targetId: 'packed-detail', position: 'before' } as const;",
			'</script>',
			'',
			'{#snippet empty()}<p>No images yet.</p>{/snippet}',
			'{#snippet fields()}<label>Hero image <input name="hero" /></label>{/snippet}',
			'{#snippet detailFields()}<label>Detail image <input name="detail" /></label>{/snippet}',
			'{#snippet actions()}',
			'\t<ReorderActions',
			...
			"\t\t\ttext: 'Hero image, position 1 of 2'",
			'\t\t}}',
			'\t\tonMoveUp={() => {}}',
			'\t\tonMoveDown={() => {}}',
			'\t/>',
			...
			'\t\tdrag={{',
			"\t\t\tid: 'packed-detail',",
			"\t\t\tlabel: 'Drag packed detail image',",
			'\t\t\tcandidate: packedCandidate,',
			'\t\t\tonDrop: () => {}',
			'\t\t}}',
			'\t/>',
	...
	await writeFile(
		...
		[
			...
			"\t'aria-label=\"Drag packed hero image\"',",
			"\t'data-giu-drop-candidate=\"before\"',",
			"\t'aria-label=\"Move hero image up\"',",
			"\t'aria-label=\"Move hero image down\"',",
			"\t'id=\"packed-hero-reorder-context\"',",
			"\t'aria-label=\"Drag packed detail image\"',",
			"\t'Hero image, position 1 of 2',",
			...
			"\teditableListMarkup.includes('draggable=') ||",
			"\teditableListMarkup.includes('aria-grabbed') ||",
			"\teditableListMarkup.includes('aria-dropeffect') ||",
			'\t(editableListMarkup.match(/type="button"/g) ?? []).length !== 4',
			') {',
	...
	await writeFile(
		...
		[
			...
			"const editableListRowDropPosition: EditableListRowDropPosition = 'before';",
			"const editableListRowDragCandidate: EditableListRowDragCandidate = { sourceId: 'packed-hero', targetId: 'packed-detail', position: editableListRowDropPosition };",
			"const editableListRowDragCancelReason: EditableListRowDragCancelReason = 'escape';",
			"const editableListRowDrag: EditableListRowDrag = { id: 'packed-hero', label: 'Drag packed hero image', candidate: { sourceId: 'packed-hero', targetId: 'packed-detail', position: 'after' }, onDragStart: (sourceId) => { void sourceId; }, onDragCandidate: (candidate) => { void candidate; }, onDrop: (candidate) => { void candidate; }, onDragCancel: (reason) => { void reason; } };",
			'const editableListRowProps: EditableListRowProps = { position: 1, fields: rowFields, drag: editableListRowDrag };',
			...
			'// @ts-expect-error EditableListRowDrag id is required.',
			"const missingEditableListRowDragId: EditableListRowDrag = { label: 'Drag image', onDrop: () => {} };",
			'// @ts-expect-error EditableListRowDrag label is required.',
			"const missingEditableListRowDragLabel: EditableListRowDrag = { id: 'packed-hero', onDrop: () => {} };",
			'// @ts-expect-error EditableListRowDrag onDrop is required.',
			"const missingEditableListRowDragDrop: EditableListRowDrag = { id: 'packed-hero', label: 'Drag image' };",
			'// @ts-expect-error EditableListRowDropPosition is closed.',
			"const invalidEditableListRowDropPosition: EditableListRowDropPosition = 'inside';",
			'// @ts-expect-error EditableListRowDragCandidate sourceId is required.',
			"const missingEditableListRowDragCandidateSource: EditableListRowDragCandidate = { targetId: 'packed-detail', position: 'before' };",
			'// @ts-expect-error EditableListRowDragCancelReason is closed.',
			"const invalidEditableListRowDragCancelReason: EditableListRowDragCancelReason = 'drop';",
			...
			'void editableListRowDropPosition;',
			'void editableListRowDragCandidate;',
			'void editableListRowDragCancelReason;',
			...
			'void missingEditableListRowDragId;',
			'void missingEditableListRowDragLabel;',
			'void missingEditableListRowDragDrop;',
			'void invalidEditableListRowDropPosition;',
			'void missingEditableListRowDragCandidateSource;',
			'void invalidEditableListRowDragCancelReason;',
